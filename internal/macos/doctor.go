...
func (d Doctor) services(ctx context.Context) report.Evidence {
	...
	data := report.ServicesData{Running: make(map[string]bool, len(names))}
	for _, name := range names {
		result := d.Runner.Run(ctx, "/usr/bin/pgrep", "-x", name)
		data.Running[name] = result.Err == nil && strings.TrimSpace(result.Stdout) != ""
	}
	return report.Evidence{ID: report.EvidenceServices, Status: report.StatusOK, Summary: "Checked core macOS application and security services", Data: data}
}
